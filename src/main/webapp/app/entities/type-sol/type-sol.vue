<template>
  <div>
    <h2 id="page-heading" data-cy="TypeSolHeading">
      <span v-text="$t('pfaApp.typeSol.home.title')" id="type-sol-heading">Type Sols</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('pfaApp.typeSol.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'TypeSolCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('pfaApp.typeSol.home.createLabel')"> Create a new Type Sol </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && typeSols && typeSols.length === 0">
      <span v-text="$t('pfaApp.typeSol.home.notFound')">No typeSols found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="typeSols">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('pfaApp.typeSol.libelle')">Libelle</span></th>
            <th scope="row"><span v-text="$t('pfaApp.typeSol.type')">Type</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="pfaApp.typeSol.delete.question" data-cy="typeSolDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-typeSol-heading" v-text="$t('pfaApp.typeSol.delete.question', { id: removeId })">
          Are you sure you want to delete this Type Sol?
        </p>
      </div>
